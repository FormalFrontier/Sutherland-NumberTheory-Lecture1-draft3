...
4. For item-level progress, update `progress/status.json`
...
After Stage 3.2 scaffolding review, every blob undergoes a Stage 3.3 missing-claims audit — a deliberately skeptical re-read that catches asides, remarks, and observations that should have been formalized. A blob is not ready for proof work until it has reached `claims_audited`. See PLAN.md.
...
- Item-level: `progress/status.json`
