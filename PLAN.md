...
This document describes the pipeline for formalizing a mathematics textbook into Lean 4 with Mathlib. Work through the stages in order. Record progress in per-turn files in `progress/` and item-level status in `progress/status.json`. Do not modify this file for progress tracking.
...
Before beginning proof work (Stage 3.4) on an item, a separate verification issue must be created so a different agent can review the scaffolding skeptically. This gate helps prevent mis-formalizations and definition-level sorries.

Can begin per-item as soon as that item's scaffolding (Stage 3.1) is merged.

The review agent should update `progress/status.json` with the status transition, and submits a PR with any fixes. The review must perform an explicit **coverage audit**, included in the PR body as a checklist:
...
### Stage 3.3: Missing Claims Audit

A deliberately skeptical re-read of every blob's prose, done after Stage 3.2 scaffolding review has merged. The goal is to catch mathematically formalizable claims — definitions, characterizations, constructions, equivalences, and asides like "note that…", "it is easy to see…", "in particular…" — that scaffolding agents missed during Stage 3.1. A single pass at scaffolding time is not enough: an auditor reading the blob again, with neighbouring Lean declarations visible, tends to notice claims buried in prose that the original scaffolder skipped.

Audit *work* is batched per issue; audit *state* is per-item. Items in the same batch do not block each other.

#### When to create audit issues

An audit issue can be created once every item it covers has reached `definition_verified` (or the terminal `non_formalizable`). Orchestrators should create audit issues for a chapter in a single burst once the chapter is ready, so auditors can be dispatched in parallel.

**Batching:** Each audit issue covers **at most 10 consecutive blobs** (in book order). Batches must not cross chapter boundaries — frontmatter, each chapter, and backmatter get their own sets of audit issues. Large chapters produce many audit issues; a 1000-blob chapter needs ~100 audit issues. Consecutive-blob batching preserves context for short-range back-references ("…as noted above…") without forcing a re-read of the entire chapter.

Create `gh label create missing-claim --color d73a4a` and `gh label create missing-claim-audit --color d73a4a` (ignore errors if they already exist). Each audit issue:
- Title: `Audit blobs <first-blob-id> .. <last-blob-id>`
- Body: list of blob IDs in the batch, links to each blob and its `.lean` file, and pointers to `research/mathlib-coverage.json` / `research/external-sources.json` for context
- Label: `missing-claim-audit`

#### Auditor workflow

Each agent assigned to an audit issue:

1. **Reads the full context.** Every blob in the batch, every blob's `.lean` file (if one exists), and the transitive import graph of earlier items. This matters: detecting "already formalized in an imported earlier item" requires knowing what the imports carry, and detecting "missing import" requires knowing the full chain.
2. **Reads each assigned blob sentence by sentence.**
3. **Classifies every mathematically formalizable claim** as one of:
   - (a) formalized in this item's own `.lean` file;
   - (b) formalized in an earlier item's `.lean` file that this file already imports or `recall`s;
   - (c) formalized in an earlier item's `.lean` file but **not** imported/cited here — a **missing-import gap**;
   - (d) not formalized anywhere — a **missing-declaration gap**.

   Restatements, motivation, notation shorthand already handled, and historical or biographical asides are not claims. When in doubt, the auditor errs toward calling it a claim.
4. **Groups gaps by target blob** and files one GitHub issue per blob with gaps (not one per claim):
   - Label: `missing-claim`
   - Title: `<ItemID>: N missing claim(s)`
   - Body: one sub-section per gap, each containing the exact sentence(s) from the blob, the gap classification (missing-import vs. missing-declaration), a Lean-shaped sketch (signature only — no proof), the target `.lean` file, and a suggestion for any `research/*.json` / `.refs.md` entries that will need updating.
   - For blobs previously marked `non_formalizable` that actually have claims, use label `scaffolding` instead (the blob needs a fresh `.lean` file from scratch), and include an enumerated claim list to seed the Stage 3.1 re-run.
5. **Submits a PR** that (a) advances every clean blob to `claims_audited` in `progress/status.json`, (b) drops the status of every blob with gaps back to `needs_definition`, and (c) writes a summary file `progress/audits/<batch-id>.md` (e.g., `progress/audits/chapter-3-blobs-041-050.md`) listing every blob audited, every claim considered, the classification for each, and links to the filed issues.

#### Missing-claim fixer workflow

Each `missing-claim` issue is a small scaffolding task. The fixing agent:

1. Reads the blob, the target `.lean` file, and the issue body.
2. For **missing-import gaps:** adds the `import` and any `recall`/doc-comment needed to cite the earlier declaration. No new declaration created.
3. For **missing-declaration gaps:** adds the declaration to the target `.lean` file, following Stage 3.1 conventions — definitions have real bodies, proof obligations within definitions may be `sorry`, theorem statements may have `sorry` proofs.
4. **Updates Phase 2 artifacts.** Any newly added declaration needs corresponding entries in `research/mathlib-coverage.json`, `research/external-sources.json`, and the blob's `.refs.md` file. If the addition requires substantial new research, file a Stage 2.3 / 2.4 follow-up issue rather than doing that research inline.
5. Submits the PR with auto-merge enabled.

For `non_formalizable` reclassification (label `scaffolding`), the fixing agent performs a full Stage 3.1 scaffolding pass on that blob — creating the `.lean` file, importing dependencies, writing definitions with real bodies, etc. — and the result flows back through Stage 3.2 and then Stage 3.3 like any other newly-scaffolded blob.

#### Re-audit after fixes

A `missing-claim` fix PR is **not** self-reviewing. After the fix PR merges, the orchestrator creates a re-audit issue targeting the same blob (label `missing-claim-audit`), and a **different agent** re-reads the blob against the updated `.lean` file. The re-audit agent either advances the blob to `claims_audited` or files further `missing-claim` issues. This preserves Stage 3.2's "different agent reviews scaffolding" gate where it matters most.

To reduce churn, a single re-audit issue may cover all fix PRs merged against a given blob in the last 24 hours.

#### Coverage of Frontmatter / Backmatter / Introduction / Discussion blobs

All first-class blob types are audited, not just numbered items. Chapter introductions (`Chapter{N}/{NN}_00_Introduction`), discussion blobs (`Chapter{N}/{NN}_{MM}a_Discussion`), remarks, notes, and every frontmatter / backmatter blob are first-class. For each of these, the target file for any added declaration is **that blob's own `.lean` file** (created by Stage 3.1 unless the blob was `non_formalizable`). There is no concept of "parent theorem's `.lean` file" — every formalizable blob has its own item ID and its own `.lean` file, as established by Stages 1.6 / 1.7 / 3.1.

#### Status transitions

After Stage 3.3:
- Blob passes cleanly → `claims_audited`.
- Blob has open `missing-claim` issue(s) → `needs_definition`, with the issues tracked. Returns to `claims_audited` only after every open issue is closed and a re-audit passes.
- Blob previously marked `non_formalizable` found to have claims → `needs_definition`; the `non_formalizable` assessment in `progress/status.json` is invalidated with an explanatory note, and a `scaffolding` issue is filed.

**No item advances to `proof_formalized` until it has first reached `claims_audited`.** This is the new precondition for Stage 3.4.

**Output:**
- GitHub issues (label `missing-claim`) — one per blob with gaps, each listing all that blob's gaps
- GitHub issues (label `scaffolding`) — one per `non_formalizable` blob being reclassified
- Updated `progress/status.json` — `claims_audited` for clean blobs, `needs_definition` for blobs with gaps
- Summary files `progress/audits/<batch-id>.md` — one per audit issue
- Updated `research/*.json` and `.refs.md` files from `missing-claim` fix PRs

**Verify:**
- No item remains at `definition_verified` with open audit issues against it
- Every item at `proof_formalized` or later has at some point reached `claims_audited`
- Every `missing-claim` issue is linked from its audit batch's `progress/audits/<batch-id>.md`

### Stage 3.4: Formalization Work Loop

Orchestrate agents to formalize items, one issue per item. Proof work on an item can begin as soon as it reaches `claims_audited` (Stage 3.3).
...
Once Stages 3.2 and 3.3 are complete, all statements and definitions are verified and the prose has been audited for missing claims. Agents should prioritize the hardest, most impactful items, not items with the easiest dependency graphs.
...
Status updates are tracked in `progress/status.json`. Non-formal nodes (discussion, external dependencies) are also tracked there.
...
1. **Scan for definition-level sorries (regression check).** Re-run the Stage 3.2 scaffolding check and, for blobs touched by recent `missing-claim` fix PRs, the Stage 3.3 audit check. New definition-level sorries or freshly-missed claims can be introduced during proof work when agents add helper definitions with sorry bodies, or during missing-claim fixes when new declarations leak in without a re-audit. Any found must become issues — these are the highest priority, since they make downstream theorem statements vacuous.
...
5. **Update `progress/status.json`** to reflect the current state, especially items whose status has changed but weren't updated.
...
### Stage 3.5: Dependency Trimming (post-formalization)
...
Can begin per-item as soon as the item is sorry-free — does not need to wait for all formalization to complete. Overlaps with the Stage 3.4 work loop on other items.
...
| `claims_audited` | Stage 3.3 | Missing-claims audit passed for this blob (no open gaps) |
...
- `non_formalizable` — discussion blob assessed as containing no formalizable claims (must include enumerated claims and exclusion reasons; subject to Stage 3.2 review; Stage 3.3 may also invalidate this assessment if the audit finds claims, in which case the blob returns to `needs_definition` and is re-scaffolded)
...
- Item-level progress goes in `progress/status.json`.
