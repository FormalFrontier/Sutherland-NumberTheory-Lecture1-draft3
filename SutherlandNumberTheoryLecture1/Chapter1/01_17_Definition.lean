...
The lecture defines an element `b : B` to be integral over the base ring `A`
when it is a root of a monic polynomial in `A[X]`. Mathlib packages exactly
this notion as `IsIntegral A b`.
...
/-- The lecture's element-level notion is exactly Mathlib's predicate `IsIntegral`. -/
theorem integral_element_iff (b : B) :
    IsIntegral A b ↔ ∃ p : Polynomial A, Polynomial.Monic p ∧ Polynomial.aeval b p = 0 :=
  Iff.rfl

/-- The lecture's extension-level notion is `Algebra.IsIntegral A B`. -/
