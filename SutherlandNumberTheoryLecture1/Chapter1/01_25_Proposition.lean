...
Mathlib already packages both ingredients: the fraction-field dichotomy
`ValuationRing.iff_isInteger_or_isInteger` and the resulting
`IsIntegrallyClosed` instance. This file exposes those two facts under local
names matching the lecture's presentation.
-/
...
theorem valuationRing_isIntegrallyClosed [ValuationRing A] : IsIntegrallyClosed A :=
  inferInstance
